...
for iwfn in range(nReps):
    ...
    print 'average vref from this sim:',averaged_vref[-1]*au2wn
    plt.subplot(311)
    ...
    for tauDW in [descendantSteps]:#,2*descendantSteps,3*descendantSteps,4*descendantSteps,5*descendantSteps]:
        ...
        for iname,val in zip(InternalNames,Values):
            print iname, val
        WfnFixedNode.exportCoords(finalCoords,'Wfn-DODOD-Tau/DODOD-Start-Ground-Propagate-Asym-DW-Asym-'+parameterString+'Eq-'+str(iwfn)+'.xyz',descendantWeights)
