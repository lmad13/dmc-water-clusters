...
Wfn.setNodalSurface('Z-displacement',side='Both')
...
plotFileName=Destination+'Vref-Pop-histogram-Excited-Zdisplacement'+parameterString
plt.figure(1)
plt.subplot(311)
...
RnCoord,swapped=Wfn.molecule.calcRn(equilibratedCoordinates)
...
longRn=(np.absolute(RnCoord)>2.0)
fileout=open('GroundLongCoordinatesforZdisplacement.xyz','w')
nLongCoords=0
for n,hasLongRn in enumerate(longRn):
    if hasLongRn:
        nLongCoords=nLongCoords+1
        WfnGround.molecule.printCoordsToFile([equilibratedCoordinates[n]],fileout)
fileout.close()
print 'There are ',nLongCoords,' long coordinates'
...
for iwfn in range(nReps):
    ...
    for tauDW in [descendantSteps]:#,2*descendantSteps,3*descendantSteps,4*descendantSteps,5*descendantSteps]:
        ...
        for ides in range(nRepsDW):
            print 'DW Rep Number',ides,
            v_ref_DW_list,pop_DW_list,DWFinalCoords,descendantsTemp=Wfn.propagate(finalCoords,tauDW,initialPop=N_size,printCensus=False)
            descendantWeights=descendantWeights+descendantsTemp
        descendantWeights=descendantWeights/nRepsDW
        ##inputx=finalCoords
        parameterString=str(N_size)+'-'+str(nReps)+'-'+str(tauDW)+'-'+str(nRepsDW)
        Wfn.exportCoords(finalCoords,'Wfn-HOHOH-Tau/HOHOH-ExcitedZ-Displacement-'+parameterString+'Eq-'+str(iwfn)+'.xyz',descendantWeights)
        ...
        print 'z displacment info:', np.min(RnCoord),np.max(RnCoord),np.average(RnCoord),np.average(RnCoord,weights=descendantWeights),np.std(RnCoord)
