...
Rn2DHists=np.zeros((nBins,nBins))
averaged_vref=[]
list_of_pop_list=[]


WfnFixedNode=dmc.wavefunction('HOHOH',N_size)
WfnFixedNode.setNodalSurface('Z-displacement',side='Both')
WfnFixedNode.setIsotope('fullyDeuterated')

Wfn_deltaRFixedNode=dmc.wavefunction('HOHOH',N_size)
Wfn_deltaRFixedNode.setNodalSurface('SharedProton',side='Both')
Wfn_deltaRFixedNode.setIsotope('fullyDeuterated')
...
plotFileName=Destination+'z-displacement-startFromGround-'+parameterString
plt.figure(1)
#plt.subplot(311)
...
for iwfn in range(nReps):
    ...
    for tauDW in [descendantSteps]:#,2*descendantSteps,3*descendantSteps,4*descendantSteps,5*descendantSteps]:
        ...
        deltaRCoord,swapped=Wfn_deltaRFixedNode.molecule.calcRn(finalCoords)
        Hist2D,xedge,yedge=np.histogram2d(RnCoord, deltaRCoord, bins=nBins, range=[[-1.5,1.5],[-1.5,1.5]], normed=True, weights=descendantWeights)
        Rn2DHists=Rn2DHists+Hist2D
        ...
        for iname,val in zip(InternalNames,Values):
            print iname, val
        WfnFixedNode.exportCoords(finalCoords,'Wfn-DODOD-Tau/DODOD-Start-Ground-Propagate-ZDisp-DW-ZDisp'+parameterString+'Eq-'+str(iwfn)+'.xyz',descendantWeights)
