...
for iwfn in range(nReps):
    ...
    GatherExpectationMagMu.append(np.sum(np.linalg.norm(Dipole)*descendantWeights)/np.sum(descendantWeights))
    GatherExpectationMagMu2.append(np.sum(np.linalg.norm(Dipole)**2*descendantWeights)/np.sum(descendantWeights))
    print 'Rn^2:',GatherExpectationRn2[-1]
    print 'Rn:  ',GatherExpectationRn[-1]
    print 'Mu^2:',GatherExpectationMagMu2[-1]
    print 'Mu:  ',GatherExpectationMagMu
